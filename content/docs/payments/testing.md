...
* The Functional URL is configured properly in the Webhooks settings under each of your Projects in the PaySuper Dashboard.

## Test card numbers 

To test a bank card payment in the Sandbox you can enter an arbitrary expiry date (the format is MM/YY), a Secure code (CVC/CVV, the format is 3 digits) and a cardholder name (the format is first name last name).

{{< hint warning >}}
You can also specify incorrect details (card number, expiration date, or CVV) to generate an error.
{{< /hint >}}

You will be redirected to the Sandbox page to select the desired results for success or failure. You can click the result button to close the current page and be redirected to the Checkout Form to view the purchase status.
...
You can use any of the following test card numbers:
...
3-D Secure is emulated with a page that doesn’t require any passwords and shows 2 buttons instead. One button is for a successful authentication, another is for a failed authentication. 
...
You can choose a fixed set of currencies correspondingly to how you have configured your Products on the Projects settings page.
...
You can [configure Webhooks](/docs/payments/webhooks/) for your Project in the PaySuper Dashboard and handle them on your server.
